...
pub fn parse_vless(link: &str) -> Result<Proxy, String> {
    if !link.starts_with("vless://") {
        return Err("invalid vless link".to_string());
    }

    let mut link = link.trim_start_matches("vless://");
    ...
    if parts.len() != 2 {
        return Err("invalid vless link format".to_string());
    }
